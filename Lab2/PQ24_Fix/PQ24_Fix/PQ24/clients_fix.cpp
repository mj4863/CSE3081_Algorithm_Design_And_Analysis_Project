...
void CLIENTS::min_heap_fix_up(int index_to_heap, HEAP_ELEMENT& heap_element) {
	int cur_priority = clients[heap_element.client_ID].priority;
	int cur_index = index_to_heap;

	while (1) {
		if ((cur_index == 1) || (cur_priority >= clients[_heap[cur_index / 2].client_ID].priority))
			/* terminate when the root is reached or the element is in its correct place */
			break;
		else {
	...
	_heap[cur_index] = heap_element;
	clients[heap_element.client_ID].index_to_heap = cur_index;
}

void CLIENTS::min_heap_fix_down(int index_to_heap, HEAP_ELEMENT& heap_element) {
	int new_priority = clients[heap_element.client_ID].priority;

	int parent_index = index_to_heap;
	int child_index = 2 * index_to_heap;
	...
	while (child_index <= last_index) {
		...
		if (new_priority <= clients[_heap[child_index].client_ID].priority)
	...
	_heap[parent_index] = heap_element;
	clients[heap_element.client_ID].index_to_heap = parent_index;
}

void CLIENTS::min_heap_insert(unsigned int client_ID) {
	HEAP_ELEMENT heap_element_to_be_inserted(client_ID);
	_heap.push_back(heap_element_to_be_inserted);

	min_heap_fix_up((int)_heap.size() - 1, heap_element_to_be_inserted);
}

unsigned int CLIENTS::min_heap_delete() {
	if (_heap.size() <= 1) return NULL_CLIENT_ID;

	int client_ID_to_be_deleted = _heap[1].client_ID;
	HEAP_ELEMENT heap_element_last = _heap[_heap.size() - 1];
	_heap.pop_back();

	min_heap_fix_down(1, heap_element_last);

	return client_ID_to_be_deleted;
}
...
void CLIENTS::min_heap_adjust_priority(int index_to_heap, int priority_increment) {
	...
	clients[client_ID].priority += priority_increment;
	HEAP_ELEMENT heap_element_to_be_adjusted = _heap[index_to_heap];

	if (priority_increment > 0)
		min_heap_fix_down(index_to_heap, heap_element_to_be_adjusted);
	else if (priority_increment < 0)  
		min_heap_fix_up(index_to_heap, heap_element_to_be_adjusted);
}
