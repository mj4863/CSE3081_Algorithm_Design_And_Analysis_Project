...
void CLIENTS::min_heap_adjust_priority(int index_to_heap, int priority_increment) {
	...
	if (priority_increment > 0) {
		...
		int child_index = index_to_heap * 2;
		int last_index = _heap.size() - 1;
		while (child_index <= last_index) {
			if ((child_index < last_index)
				&& (clients[_heap[child_index].client_ID].priority >
		...
		// fill this part!!!
		while (1) {
			if ((index_to_heap == 1) || (priority_client >= clients[_heap[index_to_heap / 2].client_ID].priority))
				break;
			else {
				_heap[index_to_heap] = _heap[index_to_heap / 2];
				clients[_heap[index_to_heap / 2].client_ID].index_to_heap = index_to_heap;
				index_to_heap /= 2;
			}
		}
		_heap[index_to_heap] = client_ID;
		clients[client_ID].index_to_heap = index_to_heap;
	}
}
